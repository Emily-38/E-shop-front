...
import { UpdateCategory, updateProduct, UpdateType } from '@/services/update';
...
export const ModalUpdateType = ({params}:ParamsProduct) => {
    ...
    const {register,handleSubmit ,formState:{errors}}=useForm<TypeType>({mode:'onSubmit'})
    const onSubmit: SubmitHandler<TypeType> = (data) => 
         UpdateType( params.id, data).then((res)=>{
         if(res.status=== 200){
...
>
  <Box sx={style} >
        <h1 className='text-center text-xl font-bold'>Update Type</h1>
