...
import { updateProduct, UpdateStock } from '@/services/update';
...
import { CreateStock } from '@/services/post';
...
export const ModalUpdateStock= ({params}:ParamsProduct) => {
    ...
    const {register,handleSubmit ,formState:{errors}}=useForm<StockType>({mode:'onSubmit'})
    const onSubmit: SubmitHandler<StockType> = (data) => 
         UpdateStock( params.id, data).then((res)=>{
         if(res.status === 200){
  ...
  <>
    ...
    <FaPen className='text-orange-950 text-lg' />
    </Button>
<Modal
  open={open}
  onClose={handleClose}
...
>
  <Box sx={style} >
        <h1 className='text-center text-xl font-bold'>Update stock</h1>
